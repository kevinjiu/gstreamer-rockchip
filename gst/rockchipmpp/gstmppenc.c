...
gst_mpp_enc_convert (GstVideoEncoder * encoder, GstVideoCodecFrame * frame)
{
  ...
  if (!outbuf) {
    GST_WARNING_OBJECT (self, "gst_mpp_enc_convert() gst_buffer_new error");  // KevinJ added for debug
    goto err;
  }
...
convert:
  ...
  if (!out_mem) {
    GST_WARNING_OBJECT (self, "gst_mpp_enc_convert() gst_allocator_alloc error");  // KevinJ added for debug
    goto err;
  }
  ...
  if (self->rotation) {
    GST_WARNING_OBJECT (self, "gst_mpp_enc_convert() self->rotation not defined");  // KevinJ added for debug
    goto err;
  }
  ...
  if (gst_video_frame_map (&src_frame, &src_info, inbuf, GST_MAP_READ)) {
    if (gst_video_frame_map (&dst_frame, dst_info, outbuf, GST_MAP_WRITE)) {
      if (!gst_video_frame_copy (&dst_frame, &src_frame)) {
        ...
        GST_WARNING_OBJECT (self, "gst_mpp_enc_convert() gst_video_frame_copy error");  // KevinJ added for debug
        goto err;
      }
      gst_video_frame_unmap (&dst_frame);
    }
